...
export function StudyView() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex overflow-x-auto gap-2 rounded-xl border border-slate-200 bg-white p-2 shadow-sm dark:border-slate-700 dark:bg-slate-800 scrollbar-none">
        {STUDY_TABS.map(({ id, label }) => (
          <button
            key={id}
            type="button"
            className={`flex-shrink-0 rounded-lg px-3 py-2 text-sm font-medium transition-colors duration-150 ${
              tab === id
